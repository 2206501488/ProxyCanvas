...
如果本地 Nanobanana2 服务运行在 `8045` 端口，可以配置。`NANOBANANA2_BASE_URL` 对应的是 Antigravity-Manager / Nanobanana2 服务地址和端口，请按实际端口修改：
...
| 要配置的内容 | 配置位置 | 说明 |
| --- | --- | --- |
| 后端端口 | `backend_v2/config.py` 的 `SERVER_PORT` | 默认 `5700`。后端启动时如果端口被占用，会尝试自动换到可用端口并写入 `server_port.json`。 |
| 前端端口 | `backend_v2/config.py` 的 `FRONTEND_PORT`，以及 `server_port.json` | 默认 `5380`。Vite 会读取 `server_port.json` 决定开发服务器端口和后端代理端口。 |
| 图片保存路径 | `backend_v2/config.py` 的 `OPENAI_SAVE_DIR` | 默认 `os.path.join(PROJECT_ROOT, "gallery")`，也就是项目根目录下的 `gallery/`。目录会在生成、导入、缩略图缓存时自动创建。 |
| APIMart | `backend_v2/config.py` 的 `API_KEY`、`API_BASE_URL` | `API_KEY` 填 APIMart 后台获取的 key；`API_BASE_URL` 默认 `https://api.apimart.ai`。 |
| ChatGPT2API / OpenAI-compatible | `backend_v2/config.py` 的 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`OPENAI_IMAGE_MODEL` | 指向本地或远程 OpenAI-compatible 图片接口，默认本地 `http://127.0.0.1:8000/v1`。 |
| CLIProxyAPI | `backend_v2/config.py` 的 `CLIPROXY_API_KEY`、`CLIPROXY_BASE_URL` | 默认本地 `http://127.0.0.1:8317/v1`。 |
| Nanobanana2 | `backend_v2/config.py` 的 `NANOBANANA2_API_KEY`、`NANOBANANA2_BASE_URL` | 默认本地 `http://127.0.0.1:8045`，对应 Antigravity-Manager 默认端口。 |
...
| 代理 | `backend_v2/config.py` 的 `HTTP_PROXIES` | 如果下载参考图或访问外部 API 需要代理，在这里设置；不需要代理可设为 `None`。 |
| 任务并发和超时 | `backend_v2/config.py` 的 `JOB_WORKER_MAX_WORKERS`、`JOB_PROVIDER_LIMITS`、`JOB_DEFAULT_TIMEOUT_SECONDS` | 控制后台任务 worker 数、不同 provider 的并发上限和任务超时时间。 |
...
图片保存路径也在 `backend_v2/config.py` 中设置：

```python
OPENAI_SAVE_DIR = os.path.join(PROJECT_ROOT, "gallery")
